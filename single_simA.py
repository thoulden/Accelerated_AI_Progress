...
def transform_sizes_to_years(sizes):
    ...
    - We estimate that each doubling of effective training compute is equivalent
      ...
      get smarter models. This model counts a doubling of "AI capabilities" as a
      doubling of the size of the AI population. So we get two doublings of AI
      capabilities per doubling of effective compute. (See "Better capabilities"
      in gdoc appendix.)
...
def plot_single_transformed_simulation(times, sizes, label, Yr_Left_sample):
    ...
    Parameters:
        ...
        Yr_Left_sample: The "years till ceiling" value, used for plotting the horizontal line.
    """
...
def run():
    # ----------------------------------------------------
    # 1) Use a session-state flag to track if we've already auto-run
    # ----------------------------------------------------
    if "initial_run_done" not in st.session_state:
        st.session_state.initial_run_done = False

    # ----------------------------------------------------
    # 2) Define the sidebar and "Run Simulation" button
    # ----------------------------------------------------
    run_simulation = st.sidebar.button('Run Simulation')

    # Parameters for the simulation (defaults set to your "median" guesses)
    lambda_sample = st.sidebar.number_input(
        'Parallelizability (λ)',
        min_value=0.01, max_value=1.0,
        value=0.4,  # median default
        step=0.01
    )
    r_0_sample = st.sidebar.number_input(
        'Initial Research Productivity (r₀)',
        min_value=0.0, max_value=5.0,
        value=1.2,  # median default
        step=0.1
    )
    Yr_Left_sample = st.sidebar.number_input(
        'Years Till Ceiling',
        min_value=1.0, max_value=50.0,
        value=10.5,  # median default
        step=0.5
    )

    # Option to enable compute growth
    compute_growth = st.sidebar.checkbox('Compute Growth')
    if compute_growth:
        f_sample_min = st.sidebar.number_input(
            'Initial Acceleration (f)',
            min_value=0.0, max_value=1000.0,
            value=0.10, step=0.1
        )
        f_sample_max = st.sidebar.number_input(
            'Max Acceleration (f)',
            min_value=f_sample_min, max_value=1000.0,
            value=32.0, step=0.1
        )
    else:
        f_sample = st.sidebar.number_input(
            'Acceleration term (f)',
            min_value=0.0, max_value=1000.0,
            value=8.0,  # median
            step=0.1
    ...
    # ----------------------------------------------------
    # 3) Wrap the existing sim + plotting in a function
    # ----------------------------------------------------
    def run_the_simulation():
        """
        Runs the dynamic system simulation and displays all resulting plots.
        """

        # ---- 3A) Parameter setup ----
        def choose_parameters():
            """
            Prepare & return parameters for the dynamic system.
            """
            if compute_growth:
                # small factor for smooth increments if compute grows
                factor_increase = 1.1
            else:
                # jump in doublings if no compute growth
                factor_increase = 2
            ...
            # incorporate the acceleration factor
        ...
        def dynamic_system_with_lambda(
            r_initial, initial_factor_increase_time, limit_years,
            compute_growth_monthly_rate, f_0, f_max,
            compute_size_start, compute_max,
            factor_increase, lambda_factor=0.5,
            max_time_months=48
        ):
            """
            Core simulation of AI capability growth over time.
            """
            ...
            # figure out total increments before diminishing returns
            ...
            while time_elapsed < max_time_months and size < ceiling and r > 0:
                ...
                # update f based on compute
                if compute_size < compute_max:
                    f = f_0 + (f_max - f_0) * (
                        np.log(compute_size / compute_size_start) / np.log(compute_max / compute_size_start)
                    )
                else:
                    f = f_max
                f_values.append(f)

                # handle retraining cost or not
                if r > 0:
                    if retraining_cost:
                        accel_factor = (
                            (lambda_factor * ((1 / r) - 1)) /
                    ...
                    factor_increase_time *= (
                        (factor_increase ** accel_factor) / ((1 + f) / (1 + f_old))
                    )

            return times, sizes, rs, ceiling, compute_sizes, f_values

        # ---- 3B) Actually run the simulation using the chosen parameters ----
        (
            factor_increase,
            r_initial,
            initial_factor_increase_time,
        ...
        # ---- 3C) Display a quick summary of parameters used ----
        st.markdown("### Simulation run with the following parameters:")
        st.write(f"- λ (Parallelizability) = {lambda_sample}")
        st.write(f"- r₀ (Initial Research Productivity) = {r_0_sample}")
        st.write(f"- Years Till Ceiling = {Yr_Left_sample}")
        st.write(f"- Compute Growth? {compute_growth}")
        st.write(f"- f (Acceleration term(s)): min={f_sample_min}, max={f_sample_max}")
        if retraining_cost:
            st.write("- Retraining Cost: **Enabled**")
        else:
            st.write("- Retraining Cost: Disabled")
        st.write("---")

        # ---- 3D) Plot the main simulation results ----
        plot_single_transformed_simulation(
            times, sizes,
            label="AI Capabilities Simulation",
        ...
        # Plot f over time if compute growth is enabled
        if compute_growth:
        ...
        # For reference: g = 2.77 => doubling every ~3 months
        g = 2.77
        multipliers = [3, 10, 30]
    ...
    # ----------------------------------------------------
    # 4) Decide if we auto-run or wait for button
    # ----------------------------------------------------
    if run_simulation:
