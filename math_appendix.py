...
def display():
  ...
  st.markdown(r""" Hence we can see that the semi-endogenous growth version of the itterative doubling time expression differs in two ways from the one employed in the post: i). the semi-engodenous set up includes an $\alpha$ term which was equal to one in the post and ii). the semi-endogenous version include a software level adjustment term, $S^{\lambda(\frac{1}{r(2S)} - \frac{1}{r(S)})}$. I will say something about each of these differences in tern
