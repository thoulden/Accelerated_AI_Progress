...
def display():
  ...
  On ii), this difference can't be rectified directly with our simple model. We can see that this term comes from the fact that $r$ is declining over time as software level grows and which implies that the exponent on $S$ after a doubling of software will decrease. Because $r$ is declining, we have that $\lambda(\frac{1}{r(2S)} - \frac{1}{S}) > 0$ which implies that, relative to the semi-engodenous growth model, our simple model will over estimate the doubling time according to the itterative doubliing time exprerssion above. We omit this terms since it complicates the model, and we prefer for the changing in $r$ over time to be thought of as a simple way of implementing a pentaly to additional software progress, rather than raken too litterally.   
  """)
