...
def calculate_summary_statistics_binary(times, conditions, software_contribution_param):
    ...
    for time_period, speed_up_factor in conditions:
        # This calculates how many doublings we need to achieve speed_up_factor × recent pace
        # for time_period months
        baseline_doublings = (time_period / 12) * (4/software_contribution_param) * speed_up_factor
        required_doublings = int(baseline_doublings)
...
def calculate_years_compressed_cdf(times_matrix, software_contribution_param, max_years=20, resolution=200):
    ...
    CDF shows probability of compressing X or fewer years into 1 year.
    """
    years_points = np.linspace(0.1, max_years, resolution)
    cdf_values = []
    
    for years in years_points:
        # For CDF, we want the probability of compressing this many years OR MORE into 1 year
        # So we need to find the maximum compression achieved by each simulation
        success_count = 0
        
        for times in times_matrix:
            # Find the maximum compression this simulation achieves in any 12-month period
            max_compression = 0
            
            for i in range(len(times)):
                # Check all possible 12-month windows starting from position i
                for j in range(i + 1, len(times)):
                    time_span = times[j] - times[i]
                    if time_span <= 12:  # Within 12 months
                        # Calculate how many doublings occurred
                        doublings = j - i
                        # Convert doublings to years of progress
                        years_compressed = doublings * (12 / time_span) * software_contribution_param / 4
                        max_compression = max(max_compression, years_compressed)
                    elif time_span > 12:
                        break  # No need to check further for this starting point
            
            # For CDF: count if this simulation can compress AT LEAST this many years
            if max_compression >= years:
                success_count += 1
        
        fraction = success_count / len(times_matrix)
        cdf_values.append(fraction)
    
    # Ensure CDF is monotonically decreasing (since we're measuring "at least X years")
    # and then flip it to make it increasing for standard CDF representation
    # Actually, we want P(can compress at least X years), which naturally decreases
    # For standard CDF format, we could calculate 1 - P(at least X) = P(less than X)
    # But for this use case, showing P(at least X) is more intuitive
    
    return years_points, cdf_values
