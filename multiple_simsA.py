...
def run():
    ...
    r_low = st.sidebar.number_input(r"Diminishing returns ($r_0$); lower bound", min_value=0.01, value=0.4, help="Each time cognitive inputs to software R&D double, how many times does software double? (Note this parameter falls over time.)")
    r_high = st.sidebar.number_input(r"Diminishing returns ($r_0$); upper bound", min_value=r_low, value=3.6)
