...
def run():
    ...
    ib_low = st.sidebar.number_input(r"Boost ($f$); lower bound", min_value=0.1, value=2.0, help="After ASARA is deployed, how many times faster does software progress become (compared to the recent pace of software progress)?")
    ib_high = st.sidebar.number_input(r"Boost ($f$); upper bound)", min_value=ib_low, value=32.0)
    r_low = st.sidebar.number_input(r"Initial Productivity ($r_0$); lower bound", min_value=0.01, value=0.4, help="Each time cognitive inputs to software R&D double, how many times does software double? (Note this parameter falls over time.)")
    r_high = st.sidebar.number_input(r"Initial Productivity ($r_0$); upper bound", min_value=r_low, value=3.6)
    ly_low = st.sidebar.number_input("Years to Ceiling; lower bound", min_value=1.0, value=5.0, help="At the start of the simulation, how far is software from effective limits? Measured in the years of AI progress at recent rates of progress.")
    ly_high = st.sidebar.number_input("Years to Ceiling; upper bound", min_value=ly_low, value=13.0)
    ...
    if run_button:
        probabilities, times_matrix, sizes_matrix, params_list = run_simulations(num_sims, conditions, r_low, r_high, ly_low, ly_high, lf_low, lf_high, ib_low, ib_high, retraining_cost, compute_growth)
